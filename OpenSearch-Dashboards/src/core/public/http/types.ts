...
export interface HttpFetchOptionsWithPath extends HttpFetchOptions {
  /*
   * The path on the Dashboards server to send the request to. Should not include the basePath.
   */
  path: string;
}
 ...
 *
 * @param path the path on the Dashboards server to send the request to. Should not include the basePath.
...
export interface HttpResponse<TResponseBody = any> {
  ...
  /** Raw request sent to Dashboards server. */

...
export async function migrateRawDocs(
  ...
  for (const raw of rawDocs) {
    if (serializer.isRawSavedObject(raw)) {
      ...
      log.error(
        `Error: Unable to migrate the corrupt Saved Object document ${raw._id}. To prevent Dashboards from performing a migration on every restart, please delete or fix this document by ensuring that the namespace and type in the document's id matches the values in the namespace and type fields.`,
        { rawDocument: raw }
      );

...
/*
 * This provides a mechanism for preventing multiple Dashboards instances from
 ...
 *
 * The reason we have to coordinate this, rather than letting each Dashboards instance
 * perform duplicate work, is that if we allowed each Dashboards to simply run migrations in
 ...
 * or someone tampering with the Dashboards alias. In these cases, it's not clear that we should
 * just migrate data into an existing index. Such an action could result in data loss. Instead,
 * we should probably fail, and the Dashboards sys-admin should clean things up before relaunching
 ...
 * other Dashboards instance to complete the migration.
 */
function handleIndexExists(error: any, log: SavedObjectsMigrationLogger) {
  ...
  log.warning(
    `Another Dashboards instance appears to be migrating the index. Waiting for ` +
      `that migration to complete. If no other Dashboards instance is attempting ` +

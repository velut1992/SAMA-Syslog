 ...
 * This is because Dashboards UI and other clients really can't be expected build the migrationVersion
...
function nextUnmigratedProp(doc: SavedObjectUnsanitizedDoc, migrations: ActiveMigrations) {
  return props(doc).find((p) => {
    ...
    // we are dealing with a document that belongs to a future Dashboards / plugin version.
    if (docVersion && (!latestVersion || Semver.gt(docVersion, latestVersion))) {
      throw Boom.badData(
        `Document "${doc.id}" has property "${p}" which belongs to a more recent` +
          ` version of Dashboards [${docVersion}]. The last known version is [${latestVersion}]`,
        doc
      );
    }
